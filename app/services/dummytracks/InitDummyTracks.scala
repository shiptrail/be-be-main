...
class LoadInitTracks @Inject()(trackService: TrackService[TrackPoint]) {
  ...
  def loadDummyTracks(listOfFiles: Array[File]): Unit = {
    for (file <- listOfFiles) {
      ...
      val (trackPointIterator1, trackPointIterator2) = MultiFormatParser.parse(file).duplicate
  ...
  def loadDummyTrack(trackPointIterator: Iterator[TrackPoint], device: UUID): Unit = {
...
class SaveDummyTracksIntoFile {
  def saveToJsonFile(trackPointIterator: Iterator[TrackPoint], device: UUID): Unit = {
    val filePath = ConfigFactory.load().getString("file.path.of.destination.for.dummyfiles")
    val id = device.toString()
    val coordinates: mutable.ListBuffer[(Double, Double, Long)] = mutable.ListBuffer()
    for (trackPoint <- trackPointIterator) {
      val coordinatesEntry = (trackPoint.lat, trackPoint.lng, trackPoint.timestamp.value)
      coordinates += coordinatesEntry
    }
    val json = JacksMapper.writeValueAsString[mutable.ListBuffer[(Double, Double, Long)]](coordinates)

    val out = new PrintWriter(new FileWriter(filePath + id + ".json"));
    out.write("{ \"tracks\" : [{ \"id\": \""
      + id
      + "\",\"coordinates\": "
      + json
