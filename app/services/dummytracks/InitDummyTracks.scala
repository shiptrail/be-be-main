...
/** *
  ...
  * All gpx-files, cgps-files, fps-files and json-files will be used. For the json-Files it will be assumed
  * that the are in the output file format of the Android-App
  * Every file will get his own random UUID and every TrackPoint in each File will be associated to
  * to that UUID. Even if there are different Tracks inside the file!
  *
  * @param trackService
  */
@Singleton
