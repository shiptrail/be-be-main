...
object FrontendFileParser extends GpsTrackParser {
  ...
  override def parse(inputStream: InputStream): Iterator[TrackPoint] = {
    ...
    val tracks =
      org.json4s.jackson.JsonMethods.parse(inputStream).extract[Tracks]
    ...
    for (track <- tracks.tracks) {
      ...
      for ((coordinate, i) <- track.coordinates.zipWithIndex) {
        val trackPoint = TrackPoint(
            coordinate(0), coordinate(1), coordinate(2).toLong.of[ms])
      ...
      for (event <- track.events) {
        for ((coordinate, i) <- event.coordinates.zipWithIndex) {
          val trackPoint = TrackPoint(
              coordinate(0),
              coordinate(1),
              coordinate(2).toLong.of[ms],
              None,
              List(),
              List(),
              List(),
              List(),
              findAnnotation(event.`type`, i, coordinate(2).toLong.of[ms]))
  ...
  case class Track(
      id: String, coordinates: Array[Array[Double]], events: Array[Event])

  case class Event(
      `type`: String, coordinates: Array[Array[Double]], description: String)

  def findAnnotation(typeOfEvent: String,
                     positionOfCoordinate: Int,
                     offset: IntU[ms]): Seq[Annotation] = {
    if (typeOfEvent.contentEquals("WENDE")) {
      positionOfCoordinate match {
        case 0 =>
          Seq(
              TrackPoint.Annotation.apply(
                  TrackPoint.AnnotationValue.StartJibe, offset))
        case 1 =>
          Seq(
              TrackPoint.Annotation.apply(
                  TrackPoint.AnnotationValue.MidJibe, offset))
        case 2 =>
          Seq(
              TrackPoint.Annotation.apply(
                  TrackPoint.AnnotationValue.EndJibe, offset))
      }
    } else {
      Seq(
          TrackPoint.Annotation.apply(
              TrackPoint.AnnotationValue.UnknownAnnotation, offset))
    }
  }
}
