...
class AndroidFileParserTest extends FlatSpec
  with Matchers
  with ScalaFutures
  with MockitoSugar {
  ...
  "A parsed minimal AndroidApp file with three Trackpoint elements" should "contain three TrackPoint objects" in {
    val fileInputStream = new FileInputStream(new File("./test/services/parser/testfiles/androidTestFile.json"))
    val trackPoints = AndroidFileParser.parse(fileInputStream)
